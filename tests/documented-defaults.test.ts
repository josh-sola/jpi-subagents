...
describe("documented defaults (README:441)", () => {
  ...
  // extensions/subagents/agent-runner.js pulls in the whole pi-coding-agent graph, and
  ...
  it(
    ...
    async () => {
      const { getGraceTurns } = await import("../extensions/subagents/agent-runner.js");
  ...
  it(
    ...
    async () => {
      const { getDefaultMaxTurns } = await import("../extensions/subagents/agent-runner.js");
  ...
  it("nested subagent depth defaults to 2", async () => {
    const { getMaxSubagentDepth } = await import("../extensions/subagents/nested-tools.js");
  ...
  it("background concurrency defaults to 10", async () => {
    const { AgentManager } = await import("../extensions/subagents/agent-manager.js");
  ...
  it("top-level spawns default to background, nested spawns to foreground", async () => {
    const { resolveAgentInvocationConfig } =
      await import("../extensions/subagents/invocation-config.js");
  ...
  it("model scope is off by default", async () => {
    ...
    const { isScopeModelsEnabled } = await import("../extensions/subagents/model-scope.js");
  ...
  it("worktree isolation is on, with a 30-day cleanup period, by default", async () => {
    const { isWorktreeIsolationEnabled, getWorktreeCleanupPeriodDays } =
      await import("../extensions/subagents/worktree.js");
