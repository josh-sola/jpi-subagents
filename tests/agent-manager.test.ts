...
import { AgentManager } from "../extensions/subagents/agent-manager.js";
import type { AgentRecord } from "../extensions/subagents/types.js";

vi.mock("../extensions/subagents/agent-runner.js", () => ({
  runAgent: vi.fn(),
  resumeAgent: vi.fn(),
}));

vi.mock("../extensions/subagents/worktree.js", () => ({
...
import { resumeAgent, runAgent, type RunResult } from "../extensions/subagents/agent-runner.js";
import { addUsage } from "../extensions/subagents/usage.js";
import { isWorktreeIsolationEnabled } from "../extensions/subagents/worktree.js";
...
describe("AgentManager — lifetime usage + compaction count are eagerly initialized", () => {
  ...
  it("resume() also accumulates usage and increments compactions on the same record", async () => {
    ...
    const { resumeAgent: resumeMock } = await import("../extensions/subagents/agent-runner.js");
...
describe("AgentManager — isolation: worktree fails loud, no silent fallback", () => {
  ...
  it("awaitStartup rejects when createWorktree returns undefined; no orphan record left behind", async () => {
    ...
    const { createWorktree } = await import("../extensions/subagents/worktree.js");
  ...
  it("a stop that lands during the copy discards the worktree instead of running", async () => {
    ...
    const { createWorktree, cleanupWorktree } = await import("../extensions/subagents/worktree.js");
    let releaseCopy!: () => void;
...
describe("AgentManager — kept-worktree result note", () => {
  ...
  afterEach(async () => {
    manager?.dispose();
    const { cleanupWorktree } = await import("../extensions/subagents/worktree.js");
    vi.mocked(cleanupWorktree).mockReset();
    vi.mocked(cleanupWorktree).mockImplementation(async () => ({ hasChanges: false }));
  });
...
describe("AgentManager — dispose prunes worktree repos", () => {
  it("prunes the process cwd and every repo a worktree was created from", async () => {
    ...
    const { createWorktree, pruneWorktrees } = await import("../extensions/subagents/worktree.js");
    vi.mocked(createWorktree).mockResolvedValueOnce({
      path: "/wt/copy",
      baseSha: "abc",
      workPath: "/wt/copy",
