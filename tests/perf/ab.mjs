 ...
 *
 *   node tests/perf/ab.mjs master
 *   node tests/perf/ab.mjs HEAD~1 --rounds 5 --filter viewer
 *
...
const PERF_DIR = join("tests", "perf");
const FIXTURES = join("tests", "helpers", "perf-fixtures.ts");

function usage(message) {
  if (message) console.error(`\n  ${message}`);
  console.error(`
  Usage: node tests/perf/ab.mjs <ref> [--rounds N] [--filter substring]
...
function report(head, base, ref) {
  ...
  for (const r of rows) {
    const delta =
      r.delta == null ? "n/a" : `${r.delta >= 0 ? "+" : ""}${(r.delta * 100).toFixed(1)}%`;
    console.log(
...
try {
  ...
  mkdirSync(join(worktree, "tests", "helpers"), { recursive: true });
