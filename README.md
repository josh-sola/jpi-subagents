...
- **Persistent agent memory** — a per-agent memory directory under the global agent dir, with automatic read-only fallback for agents without write tools
...
Schedules are **session-scoped**: they reset on `/new` and restore on `/resume`. List and cancel via `/agents → Scheduled jobs` (creation is the `Agent` tool's job — there is no parallel manual-create wizard). Storage at `<agentDir>/jpi/subagents/schedules-<sessionId>.json` with PID-based file locking for cross-instance safety.
...
| `output_transcript` | `true` (or `output-transcript`) | Write this subagent's `.output` transcript; when set, overrides the `output-transcript` default. Set `false` to write no transcript file or path. Governs only the transcript — independent of `persist_session`, `isolation: worktree`, and `memory:` |
...
| `name` | string | no | A saved workflow — `<name>.js` in `.agents/workflows/` or `<agent dir>/workflows/`, carrying an `export const meta` declaration |
...
Runtime tuning values set via `/agents` → Settings (max concurrency, max foreground concurrency, default max turns, grace turns, nested depth, fallback agent, default join mode, scheduling on/off, scope models on/off, disable defaults on/off, strict agent files on/off, agent mentions on/off, output transcript on/off, tool description full/compact/custom, widget all/background/off, usage reporting on/off, cost display on/off, model display on/off, viewer markdown off/assistant/all) persist across pi restarts, in the `subagents { }` section of the shared `<agentDir>/jpi.kdl` (default `~/.pi/agent/jpi.kdl`, honors `PI_CODING_AGENT_DIR`) — the same config file every jpi plugin uses. The section is created with its defaults the first time the extension loads; `/agents` → Settings edits it in place, and it's just as easy to hand-edit.
...
Agents can have persistent memory across sessions. Set `memory: user` in frontmatter to enable it — it's the only scope:
...
Stored at `<agentDir>/agent-memory/<name>/` (default `~/.pi/agent/agent-memory/`, honors `PI_CODING_AGENT_DIR`). The former `project` and `local` scopes are gone; that frontmatter value still parses and silently maps to `user` rather than erroring, so an agent file written for an older version keeps working.

This scope previously hardcoded `~/.pi/agent-memory/`. If that legacy directory exists for an agent and the new location doesn't, it keeps being used — existing memories aren't orphaned.
