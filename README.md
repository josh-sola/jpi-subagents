...
- **Skill preloading** — inject named skills into agent system prompts, discovered from `.agents/skills/` and global locations (Pi-standard `<name>/SKILL.md` directory layout supported)
...
By default, foreground and background agents each stream their full conversation to a per-subagent transcript — a JSON-lines file at `<os-tmpdir>/pi-subagents-<uid>/<cwd>/<session>/tasks/<agent-id>.output` (owner-only `0700`, cleared on reboot). Set `output_transcript: false` on a custom agent to write no transcript path or file for it, or set `output-transcript #false` in the `subagents { }` jpi.kdl section to make transcripts opt-in project-wide (frontmatter overrides that default). This governs **only** the transcript: it is independent of `persist_session` (the pi session on disk), and it does not affect `isolation: worktree` (a worktree the agent left dirty is kept on disk, uncommitted) — set that accordingly if the goal is to keep a run off disk entirely. Background agent completion notifications render as styled boxes:
...
An unreadable or unparseable agent file is skipped, not fatal — a warning names the file and the error. If it was overriding a same-named agent, a second line names the file that loads instead. Set `strict-agent-files #true` in the `subagents { }` jpi.kdl section (or `/agents → Settings → Strict agent files`) to fail startup on a broken file instead; mid-session reloads still only warn.
...
Runtime tuning values set via `/agents` → Settings (max concurrency, max foreground concurrency, default max turns, grace turns, nested depth, fallback agent, default join mode, scope models on/off, disable defaults on/off, strict agent files on/off, agent mentions on/off, output transcript on/off, tool description full/compact/custom, widget all/background/off, usage reporting on/off, cost display on/off, model display on/off, viewer markdown off/assistant/all) persist across pi restarts, in the `subagents { }` section of the shared `<agentDir>/jpi.kdl` (default `~/.pi/agent/jpi.kdl`, honors `PI_CODING_AGENT_DIR`) — the same config file every jpi plugin uses. The section is created with its defaults the first time the extension loads; `/agents` → Settings edits it in place, and it's just as easy to hand-edit.

Missing fields fall back to the hardcoded defaults (max concurrency `10`, max foreground concurrency `0` = unlimited, default max turns unlimited, grace turns `5`, nested depth `2`, join mode `smart`, defaults enabled).
...
**Show model** (`show-model`, default `#false`): whether the widget's running rows name the model driving each agent and the thinking level it is running at:
...
**Viewer markdown** (`viewer-markdown`, default `"assistant"`): how much of the [conversation viewer](#ui)'s transcript is rendered as Markdown rather than shown verbatim.
...
`"custom"` registers your own description from `<agentDir>/agent-tool-description.md`. The file is read once at tool registration, so edits also apply on the next pi session. Dynamic parts stay live via placeholders — a static agent list would go stale the moment you add a custom agent:

```markdown
Launch an autonomous agent. Available types:
{{typeList}}

Custom agents live in .agents/agents/ or {{agentDir}}/agents/.
```

Placeholders: `{{typeList}}` (full per-agent descriptions), `{{compactTypeList}}` (first sentence each), `{{agentDir}}`, and `{{isolationGuideline}}` (expands with its own leading newline + `- ` bullet when the feature is on — place it directly after your last rule line; empty when [worktree isolation](#turning-worktrees-off) is off). Unknown placeholders are left verbatim with a stderr warning; a missing or empty file falls back to `"full"` with a warning.
...
Nothing auto-commits on your behalf. A worktree that made changes is a live checkout sitting in your temp directory — the hand-off is the directory itself, not a branch pulled out of it.
...
A worktree is locked (`git worktree lock`) for as long as the agent is running, so nothing else can mistake it for abandoned. At session teardown, alongside the ordinary `git worktree prune`, a sweep reclaims `pi-agent-*` worktrees left in tmpdir past their TTL — the crash-recovery case a plain prune can't reach on its own, since prune skips locked entries even when their directory is long gone. A worktree is removed only when **all** of these hold:

- not locked (no run is using it)
- older than `worktree-cleanup-period-days` (dir mtime), default **30**
- `git status --porcelain` is empty (nothing uncommitted)
- its HEAD is reachable from some branch (`git branch --contains` is non-empty)

Any one of those failing keeps the worktree — the sweep would rather leak a stale directory than delete someone's only copy of real work. A locked registration whose directory is already gone (a hard crash mid-run) is unlocked and pruned regardless of age, since there's no tree left to protect.
...
- **Per project** — `worktree-isolation #false` in the `subagents { }` jpi.kdl section. The `Agent` tool's `isolation` parameter disappears from the schema entirely, along with the usage-note bullet that describes it (so it costs the model no context and cannot be passed), and worktree creation is refused on every other path too: agent files and cross-extension RPC. The `/agents` generator also stops offering `isolation:` when writing a new agent file. Use it on a repo large enough that a copy costs real time and disk. The schema and the description are both built at tool registration, so they appear or disappear in the next pi session; the refusal itself takes effect immediately. `worktree-cleanup-period-days` (default `30`) is independent of this switch — it tunes the sweep, not whether worktrees can be created at all.

  Schema and prose are gated together on purpose. Leaving the bullet in would teach the model to pass a field that is no longer declared — accepted silently, then dropped — and since a refused worktree carries no note on the result, the model would have every reason to go on reporting a worktree path that was never created. A custom tool description should use the `{{isolationGuideline}}` placeholder rather than hardcoding the bullet, for the same reason.
...
src/
  ...
  settings.ts         # Persistent settings — the subagents { } section of jpi.kdl (jpi-base Config)
