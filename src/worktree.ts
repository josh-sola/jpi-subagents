 ...
 */
async function applyWorktreeInclude(pi: ExtensionAPI, topLevel: string, worktreePath: string): Promise<void> {
...
export async function createWorktree(
  ...
  return { path: worktreePath, baseSha, workPath: subdir ? join(worktreePath, subdir) : worktreePath };
}
...
async function sweepStaleWorktrees(pi: ExtensionAPI, cwd: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (!existsSync(entry.path)) {
      ...
      if (entry.locked) {
        try { await git(pi, cwd, ["worktree", "unlock", entry.path], 5000); } catch { /* ignore */ }
...
export async function pruneWorktrees(pi: ExtensionAPI, cwd: string): Promise<void> {
  ...
  } catch { /* a stale copy is not worth losing agent output over */ }
}
